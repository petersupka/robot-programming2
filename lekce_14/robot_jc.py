...
class Robot:
    def __init__(self, standard_rychlost=120, interval_pri_otocke=0.05, trvanie_pohybu=0.5, pauza_motorov=0.5):
        ...
        self.d = 0.075 # distance between wheel center and robot center
...
#            print("Error: ", e)
    ...
    def go(self, forward, angular):
        v_l = int(forward - self.d * angular)
        v_r = int(forward + self.d * angular)
...
#                je_vse_ok = set_canals(self.pin_reverse.duty_cycle, self.pin_forward.duty_cycle, rychlost)
    ...
    def pohyb_mierne_vpred(self):
        ...
        self.jed("levy", "dopredu", self.__standard_rychlost)
        self.jed("pravy", "dopredu", self.__standard_rychlost + 10)
        return True
    ...
    def otocka_vlavo(self):
        ...
        self.jed("levy", "dozadu", int(self.__standard_rychlost/1.2))
        self.jed("pravy", "dopredu", int(self.__standard_rychlost/1.2))

        while True:
            if self.__centralny_senzor_predtym[0] == 1:    # zbehne len 1x na zaciatku otocky
                self.__centralny_senzor_predtym[0] = 0
                LEDs_blink(LightsTypesEnum.ALL_LEFT_TURN_LIGHTS,LightsColorsEnum.COLOR_ORANGE, self.__interval_pri_otocke*2, 1)
            self.jed("levy", "dozadu", int(self.__standard_rychlost/1.3))
            self.jed("pravy", "dopredu", int(self.__standard_rychlost/1.3))
    ...
    def otocka_vpravo(self):
        ...
        self.jed("levy", "dopredu", int(self.__standard_rychlost/1.2))
        self.jed("pravy", "dozadu", int(self.__standard_rychlost/1.2))

        while True:
            if self.__centralny_senzor_predtym[1] == 1:    # zbehne len 1x na zaciatku otocky
                self.__centralny_senzor_predtym[1] = 0
                LEDs_blink(LightsTypesEnum.ALL_RIGHT_TURN_LIGHTS,LightsColorsEnum.COLOR_ORANGE, self.__interval_pri_otocke*2, 1)
            self.jed("levy", "dopredu", int(self.__standard_rychlost/1.4))
            self.jed("pravy", "dozadu", int(self.__standard_rychlost/1.4))
